...
<div id="main">
	...
	<div style="width: 100%;height:100%;">
		<div id="left_item">
			...
			<?php if($isLogin == 1){
				echo '<div class="setting_item">';
				echo '<label id = "userMan" class="setting_label'; if($arr[3]=='userMan'){echo ' menu_selected';} echo '" onclick="changeMenu(event)">用 户 管 理</label>';
				echo '</div>';
			}
			?>
			<?php if($isLogin == 0){
				echo '<div class="setting_item">';
				echo '<label id = "showManager" class="setting_label'; if($arr[3]=='showManager'){echo ' menu_selected';} echo '" onclick="changeMenu(event)">查 看 管 理 员</label>';
				echo '</div>';
			}
			?>
			<div class="setting_item">
			...
			?>
			<div class="setting_item">
				<label id = "aboutPage" class="setting_label <?php if($arr[3]=='aboutPage'){echo ' menu_selected';} ?>" onclick="changeMenu(event)">关 于 系 统</label>
			</div>
		</div>
		<div id="right_content">
			<?php 
			if($arr[3]=='searchDevices'){
				if(count($arr) == 4){
					$this->load->view('showDev/showDevs');
				}else if(count($arr) == 5){
					$this->load->view('showDev/showDevInfoPage');
				}
			}else if($arr[3]=='addDevices'){
				$this->load->view('addDevices/addDevices');
			}else if($arr[3]=='manDevices'){
				if(count($arr) == 4){
					$this->load->view('manDevices/manDevices');
				}else if(count($arr) == 5){
					$this->load->view('manDevices/changeDevInfo');
				}
			}else if($arr[3]=='checkDevices'){
				if(count($arr) == 4){
					$this->load->view('checkDevices/checkDevices');
				}else if(count($arr) == 5){
					if($arr[4] == "checkeded" || $arr[4] == "no_checked" || $arr[4] == "all_devs" || $arr[4] == "lost_devs" || $arr[4] == "initaillize" || $arr[4] == "old_devs" || $arr[4] == "available_devs"){
						$this->load->view('checkDevices/checkDevices');
					}else{
						$this->load->view('checkDevices/showDevInfoPage');
					}
				}
			}else if($arr[3]=='logMan'){
				$this->load->view('logMan/logMan');
			}else if($arr[3]=='userMan'){
				if(count($arr) == 4){
					$this->load->view('userMan/userMan');
				}else if(count($arr) == 5){
					if($arr[4] == "addUserPage"){
